...
## 3. Architecture
The plugin uses several components as shown below;
...
To achive this, our top component (`SortingTaskNode/index.tsx`) has two functions.
...
In addition we have `loadState()` to get our saves state back from Slate.js. This function is only called on the initial load to populate our own internal react state with the saved state.