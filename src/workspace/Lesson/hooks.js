...
export function useBootstrap(id, courseId, dispatch, dispatchUserAction) {
	...
	async function registerHandler() {
		...
		editorWS.on(
			"lesson/:lessonId/sections/diff patched",
			dispatchSectionDiff,
		)
	}
	...
	store.lesson.sections.forEach(section => {
		...
		section.changed.forEach(key => {
			if (key === "docValue") {
				...
				sectionDiff = buildDiff(section.savedDocValue, section.docValue)
			} else if (key === "notes") {
		...
		if (sectionDiff) {
			savePromises.push(
				new Promise(async (resolve, reject) => {
					try {
						const backendResult = await editorWS.emit(
							"patch",
