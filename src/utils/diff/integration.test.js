...
test.before(async t => {
    ...
    t.context.savedMongoDiff = JSON.parse(
        fs.readFileSync(path.join(__dirname, "integration.test.mongo.json")),
    )
})
...
test("extended object", async t => {
    ...
    t.deepEqual(mongoDiff, t.context.savedMongoDiff)
})
...
test("mongo integration test", async t => {
    ...
    await t.context.db.findOneAndUpdate(
        ...
        {
            $set: t.context.savedMongoDiff.$set,
        },
    )
    ...
    const { value: updatedDoc } = await t.context.db.findOneAndUpdate(
        ...
        { $pull: t.context.savedMongoDiff.$pull },
